...
# Explicitly load .env from the script's directory to make it more robust
...
if dotenv_path.is_file():
    ...
    # Be careful not to print the actual key
    with open(dotenv_path, "r") as f:
        content_preview = f.read().split('=')[0]
        print(f"Content preview (should be OPENAI_API_KEY): '{content_preview}'")
else:
    print(".env file NOT found at the expected location.")

api_key = os.getenv('OPENAI_API_KEY')
if api_key:
    print("OPENAI_API_KEY was loaded successfully.")
    print(f"API Key preview (first 5 chars): '{api_key[:5]}...'")
else:
    print("OPENAI_API_KEY was NOT loaded. Please check the file name and content.")
print("--- END ENV DEBUGGING ---")
